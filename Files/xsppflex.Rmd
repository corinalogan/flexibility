...
Looking to infer cognition (i.e. behavioral flexibility) from behavior, we are faced with a so called inverse problem, i.e. going from (overt) observations to (hidden) causes. This constitutes a problem because typically many different processes can result in the same empirical pattern. By formulating scientific learning models as statisticalmodels, however, we can estimate which parameter values are most
compatible with the observed choices. Here, we use multilevel reinforcement learning models to analyze data from reversal learning experiments. From trial to trial, such models update the latent values of both options for the animals based on previous values and recently experienced payoffs. We parameterize our models using two parameters, phi and lambda. Phi describes the weight of recent experience and can be interpreted as the learning or updating rate of an individual. Lambda represents the exploration rate of an individual  and is sometimes also called inverse temperature. This parameter controls how sensitive choices are to differences in tha latent value of both options. As lambda gets larger, choices become more deterministic, as it gets smaller, choices become more exploratory (random choice if lambda equals 0). 
